...
function buildOpenClawSourceInstallCommand({ repo, tag, tarballUrl }) {
  ...
  return [
    ...
    '    if curl -fL --http1.1 --connect-timeout 10 --max-time 1800 -C - -o "$out" "$url"; then',
    '      if tar -tzf "$out" >/dev/null 2>&1; then',
    '        return 0',
    '      fi',
    '      echo "[openclaw] tarball 校验失败，继续断点续传..."',
    '    fi',
    '    sleep 2',
    '    i=$((i + 1))',
    '  done',
    '  return 1',
    '}',
    'if download_tarball "$OPENCLAW_TARBALL_URL" "$TMP_BASE/openclaw.tar.gz"; then',
    '  mkdir -p "$SRC_TMP"',
